...
class LogImages(Callback):
    ...
    def __init__(self, num_samples: int, freq_train, prepare_images, log_inputs, log_predictions):
        super().__init__()
    ...
    def _save_images(self, trainer, pl_module, dataset, stage):
        ...
        for i in range(self.num_samples):
            ...
            metadata_i = {key : metadata_i[key].unsqueeze(0).to(device=pl_module.device) for key in metadata_i}

            # Get prediction for this sample
            pred_tensor = pl_module(input_tensor.unsqueeze(0), target_tensor.unsqueeze(0), metadata_i).squeeze(0).to(device=pl_module.device)
        ...
        if self.log_predictions is not None:
            ...
            experiment.add_image(f"predicted_images/{stage}", pred_images_grid, global_step=curr_epoch)
